...
/**
 * Calls config engines over HTTP. Business behaviour stays in forms / workflows / rules /
 * subscription / settings — this client only orchestrates.
 */
@Component
public class ConfigEngineClient {
  ...
  public ConfigEngineClient(RestClient.Builder restClientBuilder, AdmissionProperties properties) {
  ...
  public List<String> evaluateRules(TenantScope scope, Map<String, Object> context) {
    Map<String, Object> wrapped = post(
        properties.getIntegrations().getRulesBaseUrl() + "/api/rules/evaluate",
        scope,
        context);
    if (wrapped == null) {
      return List.of();
  ...
  private static Map<String, Object> unwrap(Map<String, Object> envelope) {
    ...
    // evaluate returns { matchedActions: [...] } inside data
