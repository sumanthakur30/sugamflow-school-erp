CREATE TABLE IF NOT EXISTS hostel_record (
    id              UUID PRIMARY KEY,
...
CREATE INDEX idx_hostel_record_org_status
    ON hostel_record (organization_id, status, updated_at DESC);

CREATE INDEX idx_hostel_record_org_created
    ON hostel_record (organization_id, created_at DESC);
