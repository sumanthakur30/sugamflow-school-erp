...
@Component({
  ...
  styleUrls: ['../../shared/admin-page.scss', './attendance.component.scss'],
})
export class AttendanceComponent implements OnInit {
  ...
  reload(): void {
    this.loading = true;
    this.error = '';
    this.api.get<any>('/api/attendance/bootstrap').subscribe({
      next: (boot) => {
      ...
      error: (err) => {
        this.loading = false;
        this.error = err?.error?.message ?? err?.message ?? 'Attendance bootstrap failed';
  ...
  loadRecords(): void {
    this.api.getItems<any>('/api/attendance/records').subscribe({
  ...
  select(row: any): void {
    this.selectedId = row.id;
    this.api.get<any>(`/api/attendance/records/${row.id}`).subscribe({
