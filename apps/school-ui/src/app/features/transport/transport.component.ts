...
@Component({
  ...
  styleUrls: ['../../shared/admin-page.scss', './transport.component.scss'],
})
export class TransportComponent implements OnInit {
  ...
  reload(): void {
    this.loading = true;
    this.error = '';
    this.api.get<any>('/api/transport/bootstrap').subscribe({
      next: (boot) => {
      ...
      error: (err) => {
        this.loading = false;
        this.error = err?.error?.message ?? err?.message ?? 'Transport bootstrap failed';
  ...
  loadRecords(): void {
    this.api.getItems<any>('/api/transport/records').subscribe({
  ...
  latestApproveDelivery(): any[] {
    ...
    for (let i = intents.length - 1; i >= 0; i--) {
      if (intents[i]?.intent === 'TRANSPORT_APPROVED') {
