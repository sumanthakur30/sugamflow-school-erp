...
@Component({
  ...
  styleUrls: ['../../shared/admin-page.scss', './exam.component.scss'],
})
export class ExamComponent implements OnInit {
  ...
  loadRecords(): void {
    this.api.getItems<any>('/api/exam/records').subscribe({
  ...
  select(row: any): void {
    this.selectedId = row.id;
    this.api.get<any>(`/api/exam/records/${row.id}`).subscribe({
