...
function Start-PlatformServiceIfNeeded {
  ...
  $args = @('-jar', $jar.FullName, '--spring.profiles.active=local', "--server.port=$Port") + $ExtraArgs
  Start-Process -FilePath 'java' -ArgumentList $args -WindowStyle Minimized
  if (-not (Wait-Http "http://localhost:$Port/actuator/health" 120)) {
    Write-Warning "$Name health not confirmed on :$Port"
  } else {
...
if (-not $SkipAuthServices) {
  Start-PlatformServiceIfNeeded -Port 8085 -Name 'auth-service' -JarDir 'auth-service' -JarPrefix 'auth-service' -ExtraArgs @(
  ...
  Start-PlatformServiceIfNeeded -Port 8080 -Name 'shop-service' -JarDir 'shop-service' -JarPrefix 'shop-service' -ExtraArgs @(
    '--eureka.client.service-url.defaultZone=http://localhost:8761/eureka',
    '--eureka.instance.prefer-ip-address=true',
    '--eureka.instance.hostname=localhost',
    '--spring.cloud.config.enabled=false',
...
Write-Host ''
Write-Host 'Next: .\scripts\seed-school-demo-auth.ps1   # first time only'
Write-Host 'Then: .\scripts\start-services.ps1'
Write-Host 'Then: cd apps\school-ui; npm start'
