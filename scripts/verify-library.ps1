# Phase 10 - Library / Book Issue vertical slice verification.
...
$flag = Invoke-RestMethod -Headers $headers -Uri 'http://localhost:9090/api/subscription/feature-flags/FEATURE_LIBRARY'
$enabled = if ($null -ne $flag.data) { $flag.data.enabled } else { $flag.enabled }
if ($enabled -ne $true) { throw "FEATURE_LIBRARY not enabled: $($flag | ConvertTo-Json -Compress)" }
Write-Host 'OK   FEATURE_LIBRARY'

Write-Host ''
Write-Host '=== Bootstrap ==='
$boot = Invoke-RestMethod -Headers $headers -Uri 'http://localhost:9090/api/library/bootstrap'
...
$created = Invoke-RestMethod -Method Post -Headers $headers `
  -Uri 'http://localhost:9090/api/library/records' `
...
for ($i = 1; $i -le 8; $i++) {
  ...
  $updated = Invoke-RestMethod -Method Post -Headers $headers `
    -Uri "http://localhost:9090/api/library/records/$id/actions" `
...
Write-Host ''
Write-Host '=== LIBRARY_APPROVED delivery ==='
$approvedIntent = $null
foreach ($intent in @($app.notificationIntents)) {
  if ($intent.intent -eq 'LIBRARY_APPROVED') { $approvedIntent = $intent }
}
if (-not $approvedIntent) { throw 'LIBRARY_APPROVED intent missing' }
$delivery = @($approvedIntent.delivery)
if ($delivery.Count -lt 1) { throw 'LIBRARY_APPROVED delivery list empty' }
...
Write-Host ''
Write-Host '=== Block rule (blank bookId) ==='
$blockBody = @{
  answers = @{
    studentName = 'No Book Student'
    admissionNo = 'ADM-9998'
    bookTitle   = 'Unknown'
    bookId      = ''
    issueDate   = '2026-07-01'
    dueDate     = '2026-07-15'
...
try {
  ...
  throw 'Expected BLOCK_LIBRARY'
} catch {
  Write-Host "OK   blocked ($($_.Exception.Message))"
}

Write-Host ''
Write-Host 'Phase 10 library verification passed.'
