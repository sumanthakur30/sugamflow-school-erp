# Phase 10 - Hostel Allocation vertical slice verification.
...
$flag = Invoke-RestMethod -Headers $headers -Uri 'http://localhost:9090/api/subscription/feature-flags/FEATURE_HOSTEL'
$enabled = if ($null -ne $flag.data) { $flag.data.enabled } else { $flag.enabled }
if ($enabled -ne $true) { throw "FEATURE_HOSTEL not enabled: $($flag | ConvertTo-Json -Compress)" }
Write-Host 'OK   FEATURE_HOSTEL'

Write-Host ''
Write-Host '=== Bootstrap ==='
$boot = Invoke-RestMethod -Headers $headers -Uri 'http://localhost:9090/api/hostel/bootstrap'
...
$created = Invoke-RestMethod -Method Post -Headers $headers `
  -Uri 'http://localhost:9090/api/hostel/records' `
...
for ($i = 1; $i -le 8; $i++) {
  ...
  $updated = Invoke-RestMethod -Method Post -Headers $headers `
    -Uri "http://localhost:9090/api/hostel/records/$id/actions" `
...
Write-Host ''
Write-Host '=== HOSTEL_APPROVED delivery ==='
$approvedIntent = $null
foreach ($intent in @($app.notificationIntents)) {
  if ($intent.intent -eq 'HOSTEL_APPROVED') { $approvedIntent = $intent }
}
if (-not $approvedIntent) { throw 'HOSTEL_APPROVED intent missing' }
$delivery = @($approvedIntent.delivery)
if ($delivery.Count -lt 1) { throw 'HOSTEL_APPROVED delivery list empty' }
...
Write-Host ''
Write-Host '=== Block rule (bedNo < 1) ==='
$blockBody = @{
  answers = @{
    studentName = 'No Bed Student'
    admissionNo = 'ADM-9997'
    roomNo      = 'R-99'
    bedNo       = 0
    hostelBlock = 'Block Z'
    startDate   = '2026-07-01'
...
try {
  ...
  throw 'Expected BLOCK_HOSTEL'
} catch {
  Write-Host "OK   blocked ($($_.Exception.Message))"
}

Write-Host ''
Write-Host 'Phase 10 hostel verification passed.'
